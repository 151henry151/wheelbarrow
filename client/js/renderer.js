...
`,
  ...
  function _updateCamera(px, py) {
    ...
    if (
      ...
      // While steering (A/D/arrows), snap — fixed lerp per frame lags behind and is fps-dependent.
      // Lerp only when driving straight / forward-only so orbit→drive recenters smoothly.
  ...
  function _ownedTileHighlight(rx, ry) {
    ...
    if (!onOwned) return;
    const { x, z } = _worldXZ(tx, ty);
    const gy = _groundY(tx, ty);
    const half = T * 0.49;
    const yLine = gy + 0.14;
    const pts = [
      new THREE.Vector3(x - half, yLine, z - half),
      new THREE.Vector3(x + half, yLine, z - half),
      new THREE.Vector3(x + half, yLine, z + half),
      new THREE.Vector3(x - half, yLine, z + half),
      new THREE.Vector3(x - half, yLine, z - half),
    ...
    const fg = new THREE.PlaneGeometry(T * 0.88, T * 0.88);
