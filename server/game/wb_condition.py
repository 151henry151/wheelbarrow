...
from server.game.constants import (
    WB_DECAY_PAINT, WB_DECAY_TIRE, WB_DECAY_HANDLE,
    WB_FLAT_THRESH, WB_BREAK_THRESH, WB_HOLE_THRESH, WB_SPILL_AMT,
...
def apply_move_decay(player: dict) -> list[str]:
    ...
    # --- Hole / spill ---
    paint_cond = player["wb_paint"]
    if paint_cond < WB_HOLE_THRESH:
        bucket = player.get("bucket", {})
        if bucket:
            ratio = 1.0 - (paint_cond / WB_HOLE_THRESH)
